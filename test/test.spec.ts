...
const resolveDefaultFont = (format: Format): string => path.resolve(__dirname, `../assets/font.${format}`)

const ttfOriginalFont = fs.readFileSync(resolveDefaultFont('ttf'))
const woff2OriginalFont = fs.readFileSync(resolveDefaultFont('woff2'))
const ttfWithoutGSUB = fs.readFileSync(path.resolve(__dirname, '../assets/font-without-gsub.ttf'))
...
describe('extract', () => {
  ...
  it('should correct extract by raw symbols from font without GSUB table', async () => {
    const { meta } = await extract(
      ttfWithoutGSUB,
      { fontName: 'test-icons', raws: ['0'], formats: ['ttf'] },
    )

    expect(meta).toHaveLength(1)
    const [glyphMeta] = meta
    expect(glyphMeta.name).toEqual('0')
    expect(glyphMeta.unicode).toEqual(['0'])
  })
})
